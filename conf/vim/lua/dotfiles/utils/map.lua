...
setmetatable(map, {
	__index = function(_, map_call)
		assert(type(map_call) == "string" and string.find(map_call, "map") ~= nil, "Map method call is invalid, either not a map or not a string.")
		return function(...)
			local arg = {...}
			local map_mode
			...
			local lhs = silent and arg[2] or arg[1]
			local rhs = silent and arg[3] or arg[2]
			local mapping
			...
			if buffer then
				if unmap then
					...
					vim.api.nvim_buf_set_keymap(vim.fn.bufnr("."), map_mode, lhs, mapping, { silent = silent, noremap = noremap })
				end
			else
				if unmap then
					vim.api.nvim_del_keymap(map_mode, lhs)
				else
					vim.api.nvim_set_keymap(map_mode, lhs, mapping, { silent = silent, noremap = noremap })
				end
			end
		end
	end
