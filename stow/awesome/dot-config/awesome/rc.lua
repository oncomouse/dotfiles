...
beautiful.widget_space = {left = nil, right = " ⁞ "}
-- Hotkey formatting:
...
awful.layout.layouts = {
    awful.layout.suit.tile, layout_cm, awful.layout.suit.max,
...
local tasklist_buttons = gears.table.join(awful.button({}, 1, swap_main),
...
end, {description = "select centered monocle layout", group = "layout"}),
                              awful.key({modkey, "Shift"}, "m", function()
    awful.layout.set(awful.layout.suit.max)
end, {description = "select max layout", group = "layout"}),
                              awful.key({modkey}, "t", function()
    awful.layout.set(awful.layout.suit.tile)
end, {description = "select tiled layout", group = "layout"}),
...
awful.key({modkey}, "p", function()
    ...
    awful.spawn.with_shell("~/dotfiles/scripts/rofi/powermenu/powermenu.sh")
end, {description = "show main menu", group = "launcher"}), -- Print Screen
awful.key({}, "Print", function()
    awful.spawn.with_shell("sleep 0.2 && " ..
                               "scrot ~/'Seafile/My Library/Photos/Screenshots/%Y-%m-%d_%H%M%S-$wx$h_scrot.png'")
end), awful.key({"Shift"}, "Print", function()
    awful.spawn.with_shell("sleep 0.2 && " ..
                               "scrot -s ~/'Seafile/My Library/Photos/Screenshots/%Y-%m-%d_%H%M%S-$wx$h_scrot.png'")
end), -- Media Keys
awful.key({}, "XF86AudioPlay", function() mpris_widget:play() end), awful.key(
                                  {}, "XF86AudioStop",
                                  function() mpris_widget:stop() end),
                              awful.key({}, "XF86AudioPrev", function()
    mpris_widget:previous_track()
end), awful.key({}, "XF86AudioNext", function() mpris_widget:next_track() end),
                              awful.key({}, "XF86AudioLowerVolume",
                                        function() volume_widget:down() end),
                              awful.key({}, "XF86AudioRaiseVolume",
                                        function() volume_widget:up() end),
                              awful.key({}, "XF86AudioMute",
                                        function() volume_widget:mute() end))
...
for i = 1, 9 do
    globalkeys = gears.table.join( -- Toggle tag on focused client.
...
awful.rules.rules = {
    {
        ...
        -- Floating clients.
        {
            rule_any = {
                ...
                class = {
                    "Arandr", "Blueman-manager", "feh", "Gimp", "Gpick",
                    "Kruler", "MessageWin", -- kalarm.
                    "Sxiv", "Thunar", "Tor Browser", -- Needs a fixed window size to avoid fingerprinting by screen size.
    ...
    }, {rule = {class = "zoom"}, properties = {screen = 1, tag = "7"}},
    {rule = {class = "Zotero"}, properties = {screen = 1, tag = "8"}}
}
